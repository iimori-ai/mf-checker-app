...
### 🛠️ 改良のポイント
* **「元の残高」列を保持**: MFから出したCSVにある「残高(円)」列を表示しつつ、その右側に今回の「計算残高」を並べました。**「ここからズレているな」**というのが一目でわかります。
* **自動マッピングの賢さ**: `find_idx` という関数を追加しました。ファイルが読み込まれた瞬間、ヘッダーに「日付」や「借方」という文字が含まれていれば、**自動的にそれを選択**します。10回中9回は何も選ばずにスタートできるはずです。
* **Excelの動作保証**: Excel読み込みで一般的に使われる `openpyxl` エンジンを通すようにしました。

### ⚠️ 大事な準備（Excel対応用）
ExcelファイルをStreamlit Cloudで動かすには、GitHubのファイル一覧にある **`requirements.txt`** を開き、中に以下の1行を書き足して保存してください。
